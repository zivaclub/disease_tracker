...
export default function App() {
  ...
  // Fetch all tables from the Express Backend on startup
  ...
  const handlePublishOutbreak = async (params: any) => {
    try {
      const res = await fetch("/api/outbreaks/add", {
        method: "POST",
  ...
  const handleFetchAiOverview = async (outbreakId: string, customPrompt: string): Promise<string> => {
    const res = await fetch("/api/ai/explain-outbreak", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="min-h-screen bg-[#FAF4EE] flex flex-col relative overflow-x-hidden">
      ...
      <header className="sticky top-0 z-40 bg-white border-b-4 border-[#1A1A1A] shrink-0">
